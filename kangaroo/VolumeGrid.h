...
struct VolumeGrid
{
    ...
    VolumeGrid( const VolumeGrid<T,Target,ManagementCopyFrom>& img )
    ...
    inline __host__
    VolumeGrid(unsigned int w, unsigned int h, unsigned int d)
        :w(w), h(h), d(d)
    {
    ...
    inline __device__ __host__
    VolumeGrid(T* ptr, size_t w, size_t h, size_t d)
        : pitch(sizeof(T)*w), ptr(ptr), w(w), h(h), img_pitch(sizeof(T)*w*h)
    {
    }

    inline __device__ __host__
    VolumeGrid(T* ptr, size_t w, size_t h, size_t d, size_t pitch)
        : pitch(pitch), ptr(ptr), w(w), h(h), img_pitch(pitch*h), d(d)
    {
    }

    inline __device__ __host__
    VolumeGrid(T* ptr, size_t w, size_t h, size_t d, size_t pitch, size_t img_pitch)
