...
inline __host__
void BoundedVolumeGrid::ResetAllGridVol()
{
...
inline __host__
void BoundedVolumeGrid::InitAllBasicSDFs()
{
  for(int i=0;i!=m_w;i++)
  {
    for(int j=0;j!=m_h;j++)
...
inline __host__
void BoundedVolumeGrid::InitSingleBasicSDFWithGridIndex(
    unsigned int x,
    unsigned int y,
    unsigned int z)
{
...
inline __host__
bool BoundedVolumeGrid::InitSingleBasicSDFWithIndex(int nIndex)
{
  if( m_GridVolumes[nIndex].d !=m_nVolumeGridRes &&
      m_GridVolumes[nIndex].h !=m_nVolumeGridRes &&
      m_GridVolumes[nIndex].w !=m_nVolumeGridRes)
...
float3 BoundedVolumeGrid::SizeUnits() const
{
  return m_bbox.Size();
}

inline __device__ __host__
float3 BoundedVolumeGrid::VoxelSizeUnits() const
{
  return m_bbox.Size() / make_float3( m_w-1, m_h-1, m_d-1 );
}

inline __device__ __host__
float3 BoundedVolumeGrid::VoxelSizeUnitsGlobal(int3 max_global, int3 min_global) const
{
...
inline __host__
bool BoundedVolumeGrid::IsValid() const
{
  int nNum = 0;
...
bool BoundedVolumeGrid::CheckIfBasicSDFActive(const int nIndex) const
{
  if(m_GridVolumes[nIndex].d == m_nVolumeGridRes &&
     m_GridVolumes[nIndex].w == m_nVolumeGridRes &&
     m_GridVolumes[nIndex].h == m_nVolumeGridRes)
...
inline __host__
int BoundedVolumeGrid::GetActiveGridVolNum()
{
  int nNum = 0;
...
inline __host__ __device__
bool BoundedVolumeGrid::CheckIfVoxelExist(int x, int y, int z)
{
  int nIndex = ConvertLocalIndexToRealIndex(
        static_cast<int>(floorf(x/m_nVolumeGridRes)),
        static_cast<int>(floorf(y/m_nVolumeGridRes)),
...
inline __device__ __host__
uint3 BoundedVolumeGrid::Voxels() const
{
  return make_uint3(m_w,m_h,m_d);
}

inline  __device__
T& BoundedVolumeGrid::operator()(unsigned int x,unsigned int y, unsigned int z)
{
  int nIndex = ConvertLocalIndexToRealIndex(
        static_cast<int>(floorf(x/m_nVolumeGridRes)),
        static_cast<int>(floorf(y/m_nVolumeGridRes)),
...
inline  __device__  __host__
T& BoundedVolumeGrid::Get(unsigned int x,unsigned int y, unsigned int z)
{
  int nIndex = ConvertLocalIndexToRealIndex(
        static_cast<int>(floorf(x/m_nVolumeGridRes)),
        static_cast<int>(floorf(y/m_nVolumeGridRes)),
...
inline  __device__
float BoundedVolumeGrid::GetUnitsTrilinearClamped(float3 pos_w) const
{
...
inline __device__
float3 BoundedVolumeGrid::GetUnitsBackwardDiffDxDyDz(float3 pos_w) const
{
  /// get pose of voxel in whole sdf, in %
  float3 pos_v = (pos_w - m_bbox.Min()) / (m_bbox.Size());
...
inline __device__
float3 BoundedVolumeGrid::GetUnitsOutwardNormal(float3 pos_w) const
{
...
inline __device__ __host__
float3 BoundedVolumeGrid::VoxelPositionInUnits(int x, int y, int z) const
{
...
inline __device__ __host__
float3 BoundedVolumeGrid::VoxelPositionInUnits(int3 p_v) const
{
  return VoxelPositionInUnits(p_v.x,p_v.y,p_v.z);
}

inline __device__ __host__
float3 BoundedVolumeGrid::VoxelPositionInUnitsGlobal(
    int x, int y, int z,
...
//////////////////////////////////////////////////////
inline __host__
void BoundedVolumeGrid::CopyFrom(BoundedVolumeGrid<T, TargetDevice, Management>& rVol )
{
...
inline __host__
void BoundedVolumeGrid::CopyFrom(BoundedVolumeGrid<T, TargetHost, Management>& rVol )
{
  for(unsigned int i=0;i!= GetTotalGridNum();i++)
  {
    m_GridVolumes[i].CopyFrom(rVol.m_GridVolumes[i]);
...
inline __host__
void BoundedVolumeGrid::CopyAndInitFrom(BoundedVolumeGrid<T, TargetDevice , Management>& rVol )
{
  for(unsigned int i=0;i!= GetTotalGridNum();i++)
  {
...
inline __host__
void BoundedVolumeGrid::CopyAndInitFrom(BoundedVolumeGrid<T, TargetHost, Management>& rHVol )
{
  for(unsigned int i=0;i!= GetTotalGridNum();i++)
  {
    // skip void volum grid
...
inline __host__
void BoundedVolumeGrid::FreeMemory()
{
  for(unsigned int i=0;i!= GetTotalGridNum();i++)
  {
    if(m_GridVolumes[i].d == m_nVolumeGridRes &&
...
inline __host__
void BoundedVolumeGrid::FreeMemoryByIndex(unsigned int nIndex)
{
  if(CheckIfBasicSDFActive(nIndex) == false)
  {
...
//////////////////////////////////////////////////////
//  inline __device__
//  float3 BoundedVolumeGrid::GetShiftValue(float3 pos_w) const
//  {
//    /// get pose of voxel in whole sdf, in %
...
inline __device__
float3 BoundedVolumeGrid::GetShiftValue(float3 pos_w, float3 cam_translate) const
{
...
//  inline __device__ __host__
//  unsigned int BoundedVolumeGrid::ConvertLocalIndexToRealIndex(int x, int y, int z) const
//  {
...
inline __host__
void BoundedVolumeGrid::UpdateLocalAndGlobalShift(int3 shift_index)
{
  // ------------------------------------------------------------------------
  // updage local shift
  // ------------------------------------------------------------------------
...
inline __device__
void BoundedVolumeGrid::SetNextInitSDF(unsigned int x, unsigned int y, unsigned int z)
{
...
//////////////////////////////////////////////////////
inline __device__ __host__
void BoundedVolumeGrid::SubBoundingVolume( const BoundingBox& region)
{
...
inline __device__ __host__
unsigned int BoundedVolumeGrid::GetTotalGridNum()
{
...
// ===========================================================================
inline __host__
BoundingBox BoundedVolumeGrid::GetDesireBB(int3 GlobalIndex)
{
