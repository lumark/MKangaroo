...
void BuildPoseRefinementFromDepthmapSystemESM(
    ...
    LeastSquaresSystem<float,6>& lss, Image<float4> dDebug,
    const float c, const bool bDiscardMaxMin, const float fMinDepth, const float fMaxDepth //depth in meters
) {
    ...
    if(isfinite(depth) && depth > fMinDepth && depth < fMaxDepth)
    {
        if( dImgr.InBounds(pr(0), pr(1), 2) &&  dImgl.InBounds(pl(0), pl(1), 2) )
        {
            ...
            {
                dDebug(u, v) = make_float4(0, 1, 1, 1); // discard max and min RGB pixel value, yellow
            }
            else
            {
                // image error
                ...
                */
                ...
                const float debug = ( abs(y) + 128 ) / 255.0f;
                dDebug(u,v) = make_float4(debug, w, 0, 1); // green is good match, blue means error is huge
            }
        }
        else
        {
            dDebug(u,v) = make_float4(1, 1, 0, 1);   // pixel out of boundary, cyan
        }
    }
    else
    {
        dDebug(u,v) = make_float4(0, 0, 1, 1);  // invalid depth value, red
    }
}
...
//            if( bDiscardMaxMin && ( Il == 0 || Il == 255 || Ir == 0 || Ir == 255 ) )
//            {
//                // yellow (intensty escape)
//                dDebug(u, v) = make_float4(0, 1, 1, 1);
//            }
//            else
//            {
//                // image error
...
//                const float w =(1.0f/depth) * LSReweightTukey(y, c);
//                lss.JTJ = OuterProduct(Jl, w);
//                lss.JTy = mul_aTb(Jl, y*w);
//                lss.obs = 1;
//                lss.sqErr = y * y;

//                const float error = ( abs(y) + 128 ) / 255.0f;

//                // if error is big, w is small, it looks bule. Other wise it looks red.
//                dDebug(u,v) = make_float4(error, 0, w, 1);
//            }
//        }
//        else
//        {
//            // green (pixel over img bound)
...
__global__ void KernPoseRefinementProjectiveIcpPointPlane(
    ...
    if( isfinite(Pr.z) && Pr.z!=0 && Nr.w == 1.0f /*&& dot3(Nr,Pr) / -length3(Pr) > 0.2*/ )
    {
        // check if out of boundary
        if(dPl.InBounds(pl, 3) )
        {
            const float4 _Pl = dPl.GetNearestNeighbour(pl);
...
//                dDebug(u,v) = make_float4(debug,debug,debug,1);
            ...
            {
                // red for invalid depth
                dDebug(u,v) = make_float4(0,0,1,1);
            }
        }
        else
        {
            // cyan for image out of boundary
            dDebug(u,v) = make_float4(1,1,0,1);
        }
    }
    else
